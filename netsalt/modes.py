...
def mode_on_nodes(mode, graph):
    ...
    if abs(min_eigenvalue[0]) > 10 * quality_thresh:
        raise Exception(
...
def compute_modal_intensities(modes_df, max_pump_intensity, mode_competition_matrix):
    ...
    logging.debug(
        "First lasing threshold %s, with id %s",
...
def _get_new_D0(arg, graph=None, D0_steps=0.1, new_D0_method="linear_approx"):
...
def find_threshold_lasing_modes(
    ...
    ```
    config = {
        "new_D0_method": "standard" | "linear_approx" | "first_guess",
        "kill_modes": True | False
    }
    ```
    -
    - "standard"
